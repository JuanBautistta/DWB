...
public class SvcCartImp implements SvcCart {
	...
	public ApiResponse addToCart(Cart cart) {
		 ...
		 */
		Integer product_stock = validateProduct(cart.getGtin()); // cambiar el valor de cero por el stock del producto recuperado de la API Product 
		...
		for (Cart c : carts) {
			if (c.getGtin().trim().equals(cart.getGtin().trim())) {
				Integer totalQuantity = c.getQuantity() + cart.getQuantity();
				if (totalQuantity > product_stock) throw new ApiException(HttpStatus.BAD_REQUEST, "invalid quantity");
				repo.updateStock(totalQuantity, c.getCart_id());
	...
	private Integer validateProduct(String gtin) {
		try {
			...
			} else throw new ApiException(HttpStatus.BAD_REQUEST, "product does not exist");
		} catch(Exception e) {
